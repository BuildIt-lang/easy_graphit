...
using vertexsubset = VertexFrontier;


static void builtin_addVertex(VertexFrontier &f, int elem);
...
public:
	...
	void addVertex(int elem) {
		builtin_addVertex(*this, elem);
	}
	int32_t size_host(void) {
		return builtin_getVertexSetSize(*this);
	}
	int32_t __device__ size_device(void) {
		return *d_num_elems_input;
	}
	void swap_queues_host(void) {
		graphit_runtime::swap_queues(*this);
		format_ready = SPARSE;
	}
	void __device__ swap_queues_device(void) {
		graphit_runtime::swap_queues_device(*this);
		format_ready = SPARSE;
	}
	void swap_bitmaps_host(void) {
		graphit_runtime::swap_bitmaps(*this);
		format_ready = BITMAP;
	}
	void swap_boolmaps_host(void) {
		graphit_runtime::swap_bytemaps(*this);
		format_ready = BYTEMAP;
	}
};
