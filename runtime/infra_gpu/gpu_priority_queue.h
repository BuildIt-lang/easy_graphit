...
static void __global__ update_nodes_special(GPUPriorityQueue<int32_t>* gpq,  int32_t num_vertices, VertexFrontier output_frontier);
static void __device__ update_nodes_identify_min_device(GPUPriorityQueue<int32_t>* gpq,  int32_t num_vertices);
static void __device__ update_nodes_special_device(GPUPriorityQueue<int32_t>* gpq,  int32_t num_vertices, VertexFrontier output_frontier);

template<typename PriorityT_>
	class GPUPriorityQueue {

	public:
		GPUPriorityQueue* device_gpq;
		size_t getCurrentPriority(){
			return current_priority_;
		}
		int __device__ prio_cutoff() {
			return frontier_.priority_cutoff;
		}
		...
		void init(GraphT<int32_t> graph, PriorityT_* device_priorities, PriorityT_ initial_priority, PriorityT_ delta, NodeID initial_node = -1){
			cudaMalloc((void**)&device_gpq, sizeof(*this));

			device_priorities_ = device_priorities;
			current_priority_ = initial_priority;
			delta_ = delta;
			ready_set_dequeued = false;
			frontier_ = create_new_vertex_set(builtin_getVertices(graph));
			...
			if (initial_node != -1){
				builtin_addVertex(frontier_, initial_node);
			}
		}
		void release(void) {
...
/*
		void __device__ updatePriorityMinOld(GPUPriorityQueue<PriorityT_> * device_gpq,  PriorityT_ new_priority, VertexFrontier output_frontier, int32_t node){
			bool output = writeMin(&(device_gpq->device_priorities_[node]), new_priority);
...
*/		
		...
		bool finished(void) {
			...
			if (!ready_set_dequeued && builtin_getVertexSetSize(frontier_) == 0){
				dequeueReadySet();
		...
		bool __device__ device_finished(void) {
			if (current_priority_ == INT_MAX)
				return true;
			if (!ready_set_dequeued && device_builtin_getVertexSetSize(frontier_) == 0) {
		...
		bool __device__ finished_device(void) {
...
*/
		...
		VertexFrontier& dequeueReadySet(void){
			...
			if (builtin_getVertexSetSize(frontier_) == 0) {
				...
				cudaCheckLastError();
				swap_queues(frontier_);
				frontier_.format_ready = VertexFrontier::SPARSE;
		...
		VertexFrontier& dequeue(void) {
			return dequeueReadySet();
		}
		...
		VertexFrontier __device__ device_dequeueReadySet(void) {
			...
			if (device_builtin_getVertexSetSize(frontier_) == 0) {
				window_upper_ = current_priority_ + delta_;
				current_priority_ = INT_MAX;
				this_grid().sync();
				if (threadIdx.x + blockIdx.x * blockDim.x == 0) {
		...
		VertexFrontier __device__ dequeue_device(void) {
			return device_dequeueReadySet();
		}
...
*/
				...
				update_nodes_special_device(this, frontier_.max_num_elems, frontier_);
				this_grid().sync();
				swap_queues_device_global(frontier_);
				this_grid().sync();	
				if (threadIdx.x + blockIdx.x * blockDim.x == 0) {
					frontier_.format_ready = VertexFrontier::SPARSE;
					ready_set_dequeued = false;
	...
	for (int i = 0; i < work_per_thread; i++) {
		...
		if (node_id < num_vertices) {
			if(gpq->device_priorities_[node_id] >= gpq->current_priority_ && gpq->device_priorities_[node_id] < (gpq->current_priority_ + gpq->delta_)) {
				enqueueVertexSparseQueue(output_frontier.d_sparse_queue_output, output_frontier.d_num_elems_output, node_id);
			}
		}
	}
}
...
template <typename T>
using PrioQueue = GPUPriorityQueue<T>;
}
