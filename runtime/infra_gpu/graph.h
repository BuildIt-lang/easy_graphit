...
struct GraphT { // Field names are according to CSR, reuse for CSC
	...
	GraphT<EdgeWeightType> *blocked_graph = nullptr;
	

	GraphT* get_transposed_graph(void) {
		builtin_transpose(*this);
		return transposed_graph;
	}

	GraphT* get_blocked_graph(int block_size) {
		if (cached_block_size == block_size)
			return blocked_graph;

		if (blocked_graph)
			delete blocked_graph;
		blocked_graph = new GraphT<EdgeWeightType>();	
		block_graph_edges(*this, *blocked_graph, block_size);
...
template <typename EdgeWeightType>
static GraphT<EdgeWeightType> load_graph(std::string filename) {
	GraphT<EdgeWeightType> graph;
	int flen = strlen(filename.c_str());
	const char* bin_extension = ".graphit_bin";
	...
	graph.h_out_degrees = new int32_t[graph.num_vertices];
	for (int i = 0; i < graph.num_vertices; i++) {
		graph.h_out_degrees[i] = graph.h_src_offsets[i+1] - graph.h_src_offsets[i];	
	}
	...
	cudaMalloc(&graph.d_out_degrees, sizeof(int32_t) * graph.num_vertices);	
	...
	cudaMemcpy(graph.d_out_degrees, graph.h_out_degrees, sizeof(int32_t) * graph.num_vertices, cudaMemcpyHostToDevice);
	...
	// We are using 512 as a max of NUM_CTA
	// FIXME in the future
	cudaMalloc(&graph.strict_cta_sum, 512 * 2 * sizeof(int32_t));
