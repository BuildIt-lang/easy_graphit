...
public:
	...
	int32_t * d_vertices;	
	void insert_host(VertexFrontier &f) {
		builtin_insert(*this, f);
	}
	void retrieve_host(VertexFrontier &f) {
		builtin_retrieve(*this, f);
	}
	void __device__ insert_device(VertexFrontier &f) {
		device_builtin_insert(*this, f);
	}
	void __device__ retrieve_device(VertexFrontier &f) {
		device_builtin_retrieve(*this, f);
	}
};

using FrontierList = VertexFrontierList;
...
static VertexFrontierList new_frontier_list(int32_t max_elems) {
	return create_new_vertex_frontier_list(max_elems);
}
